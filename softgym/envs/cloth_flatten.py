...
class ClothFlattenEnv(ClothEnv):
    def __init__(self, cached_states_path='cloth_flatten_init_states.pkl', **kwargs):
        ...
        configs,states=self.get_cached_configs_and_states(cached_states_path, self.num_variations)
    ...
    def generate_env_variation(self, num_variations=1, vary_cloth_size=True):
        ...
        print("Using generate_env_variation")
    ...
    def _set_to_flatten(self):
        ...
        new_pos = np.empty(shape=(N, 4), dtype=np.float64)
    ...
    def initialize_camera(self):
        ...
        top_down_cam_pos,top_down_cam_angle=np.array([0.0, 0.4, 0.0]), np.array([-90/180*np.pi, -90 / 180. * np.pi, 0]) #for top-down image
        self.camera_params = {
                            'default_camera':
                                  {'pos': cam_pos,
                                   'angle': cam_angle,
