...
class ClothEnv(FlexEnv):
    ...
    def get_default_config(self):
        ...
        if self.action_mode in ['sawyer', 'franka']:
            ...
            # cam_pos, cam_angle = np.array([0.0, 1.0, 0.0]), np.array([-90/180*np.pi, -90 / 180. * np.pi, 0]) #for top-down image
        
        top_down_cam_pos,top_down_cam_angle=np.array([0.0, 1, 0.0]), np.array([-90/180*np.pi,  -90/ 180* np.pi, 0]) #for top-down image
        ...
        config = {
            ...
            'camera_params': {
                            'default_camera':
                                  {'pos': top_down_cam_pos,
                                   'angle': top_down_cam_angle,
    ...
    def _get_obs(self):
        ...
        if self.observation_mode == 'point_cloud':
            ...
            pos = np.zeros(shape=self.particle_obs_dim, dtype=np.float64)
